 ...
 */
 class ListsSuite {
  ...
  @Test def `sum with negative number (10pts)`: Unit = {
    assert(sum(List(-1,2,0)) == 1)
  }

  @Test def `sum of empty (10pts)`: Unit = {
    assert(sum(List()) == 0)
  }

  @Test def `sum of repeated (10pts)`: Unit = {
  ...
  @Test def `max of list with repeated numbers (10pts)`: Unit = {
    Assert.assertEquals(3, max(List(1, 1, 2, 1, 3, 3)))
  }

  @Test def `max of empty (10pts)`: Unit = {
    try {
      max(List())
      Assert.fail("No exception has been thrown.")
    }
  ...
  @Test def `max with negatives (10pts)`: Unit = {
    Assert.assertEquals(-1, max(List(-3, -2, -1)))
  }

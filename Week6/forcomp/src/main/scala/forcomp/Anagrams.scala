...
object Anagrams extends AnagramsInterface {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences match {
      case Nil => List(Nil)
      case (char, maxCount) :: tl => {
        val otherOccurs = combinations(tl)
        otherOccurs ::: (
          for {
            occurs <- otherOccurs
            count <- 1 to maxCount
          } yield (char, count) :: occurs
        )
      }
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def subtractCounts(oldOccurs: Map[Char, Int], subOccur: (Char, Int)) = {
      val (char, occur) = subOccur
      oldOccurs(char) - occur match {
        case 0 => oldOccurs - char
        case n => oldOccurs updated (char, n)
      }
    }
    y.toMap.foldLeft(x.toMap)(subtractCounts).toList
  }
